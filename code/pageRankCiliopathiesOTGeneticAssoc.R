...
distTraits = dist(pageRankDFCilia_scaled, method = 'euclidean')
...
##plot differently
Heatmap(as.matrix(dist(variantsTable, method = 'binary')))

## calculate distance based on manual phenotype annotation
phenotype = data.frame('ciliopathy' = rownames(variantsTable), 'renal' = F, 'retina' = F, 'CNS' = F, 'polydactyly' = F, 'skeletal' = F, 'hearing' = F)
phenotype$renal[grep('kidney|meckel|renal|nephro|senior|orofacio', phenotype$ciliopathy, ignore.case = T)] = T
phenotype$retina[grep('ocular|bardet|senior|usher|leber|retinitis|RP|cone', phenotype$ciliopathy, ignore.case = T)] = T
phenotype$CNS[grep('joubert|meckel|acrocallosal|hydrolethalus|orofacio|bardet', phenotype$ciliopathy, ignore.case = T)] = T
phenotype$polydactyly[grep('meckel|orofacio|acrocallosal|bardet|mckusick|hydrolethalus|ellis|short rib', phenotype$ciliopathy, ignore.case = T)] = T
phenotype$skeletal[grep('jeune|short rib|ellis|cranioe', phenotype$ciliopathy, ignore.case = T)] = T
phenotype$hearing[grep('usher|carpenter|hearing', phenotype$ciliopathy, ignore.case = T)] = T

propagationDistDF = data.frame()
for (i in 2:ncol(phenotype)){
  tmpPheno = colnames(phenotype)[i]
  tmpDist = as.vector(dist(pageRankDFCilia_scaled[rownames(pageRankDFCilia_scaled) %in% phenotype[phenotype[,tmpPheno] == T, "ciliopathy"],], 'euclidean'))
  propagationDistDF = rbind(propagationDistDF, data.frame('distance' = tmpDist, 'phenotype' = rep(tmpPheno, length(tmpDist))))
}

propagationDistAll = as.vector(dist(pageRankDFCilia_scaled, method = 'euclidean'))
propagationDistDF = rbind(propagationDistDF, data.frame('distance' = propagationDistAll, 'phenotype' = rep('all', length(propagationDistAll))))

ggplot(propagationDistDF, aes(x=phenotype, y=distance)) + geom_boxplot() + stat_compare_means(ref.group = 'all', label = 'p.format', vjust = -1, method.args = list(alternative = 'less'))


seedDistDF = data.frame()
for (i in 2:ncol(phenotype)){
  tmpPheno = colnames(phenotype)[i]
  tmpDist = as.vector(dist(variantsTable[rownames(variantsTable) %in% phenotype[phenotype[,tmpPheno] == T, "ciliopathy"],], 'binary'))
  seedDistDF = rbind(seedDistDF, data.frame('distance' = tmpDist, 'phenotype' = rep(tmpPheno, length(tmpDist))))
}

seedDistAll = as.vector(dist(variantsTable, method = 'binary'))
seedDistDF = rbind(seedDistDF, data.frame('distance' = seedDistAll, 'phenotype' = rep('all', length(seedDistAll))))

ggplot(seedDistDF, aes(x=phenotype, y=distance)) + geom_boxplot() + stat_compare_means(ref.group = 'all', label = 'p.format', vjust = -1, method.args = list(alternative = 'less'))




pageRankDFCilia_selected = pageRankDFCilia[rownames(pageRankDFCilia) %in% phenotype$ciliopathy,]
pageRankDFCilia_scaled = t(scale(t(pageRankDFCilia_selected)))
propagationDist = as.matrix(dist(pageRankDFCilia_selected, method = 'manhattan'))

variantCertainCilia_selected = variantCertainCilia[variantCertainCilia$diseaseName %in% c(phenotype$ciliopathy,'cilia-associated RP'),]
variantsTable = variantCertainCilia_selected[,c("targetId", "diseaseId")]
variantsTable = unique(variantsTable[,1:2])
variantsTable$mutated = 1
variantsTable = spread(variantsTable,key = diseaseId, value = mutated)
variantsTable = column_to_rownames(variantsTable, var = 'targetId' )
...
test = combn(phenotype$ciliopathy, 2)
...
for (i in 1:nrow(test)) {
  if (phenotype[phenotype$ciliopathy == test[i, 1], "renal"] == T &
      phenotype[phenotype$ciliopathy == test[i, 2], "renal"] == T) {
    test$renal[i] = T
  } 
  if (phenotype[phenotype$ciliopathy == test[i, 1], "CNS"] == T &
             phenotype[phenotype$ciliopathy == test[i, 2], "CNS"] == T) {
    test$CNS[i] = T
  } 
  if (phenotype[phenotype$ciliopathy == test[i, 1], "hearing"] == T &
             phenotype[phenotype$ciliopathy == test[i, 2], "hearing"] == T) {
    test$hearing[i] = T
  } 
  if (phenotype[phenotype$ciliopathy == test[i, 1], "polydactyly"] == T &
             phenotype[phenotype$ciliopathy == test[i, 2], "polydactyly"] == T) {
    test$polydactyly[i] = T
  } 
  if (phenotype[phenotype$ciliopathy == test[i, 1], "retina"] == T &
             phenotype[phenotype$ciliopathy == test[i, 2], "retina"] == T) {
    test$retina[i] = T
  } 
  if (phenotype[phenotype$ciliopathy == test[i, 1], "skeletal"] == T &
             phenotype[phenotype$ciliopathy == test[i, 2], "skeletal"] == T) {
    test$skeletal[i] = T
  }
}

for (i in 1:nrow(test)){test$propDist[i] = propagationDist[test[i,1], test[i,2]]}
for (i in 1:nrow(test)){test$seedDist[i] = seedDist[test[i,1], test[i,2]]}

testRenal = test[(test$V1 %in% phenotype[phenotype$renal == T, "ciliopathy"] | test$V2 %in% phenotype[phenotype$renal == T, "ciliopathy"]), ]
testCNS = test[(test$V1 %in% phenotype[phenotype$CNS == T, "ciliopathy"] | test$V2 %in% phenotype[phenotype$CNS == T, "ciliopathy"]), ]
testhearing = test[(test$V1 %in% phenotype[phenotype$hearing == T, "ciliopathy"] | test$V2 %in% phenotype[phenotype$hearing == T, "ciliopathy"]), ]
testretina = test[(test$V1 %in% phenotype[phenotype$retina == T, "ciliopathy"] | test$V2 %in% phenotype[phenotype$retina == T, "ciliopathy"]), ]
testpolydactyly = test[(test$V1 %in% phenotype[phenotype$polydactyly == T, "ciliopathy"] | test$V2 %in% phenotype[phenotype$polydactyly == T, "ciliopathy"]), ]
testskeletal = test[(test$V1 %in% phenotype[phenotype$skeletal == T, "ciliopathy"] | test$V2 %in% phenotype[phenotype$skeletal == T, "ciliopathy"]), ]

rocRes = data.frame('seedDist' = NA, 'propDist' = NA, phenotype = c('renal', 'retina', 'CNS', 'hearing', 'polydactyly', 'skeletal'))
rocRes[rocRes$phenotype == 'CNS', 'seedDist'] = pROC::roc(testCNS$CNS, testCNS$seedDist)$auc
rocRes[rocRes$phenotype == 'CNS', "propDist"] = pROC::roc(testCNS$CNS, testCNS$propDist)$auc
rocRes[rocRes$phenotype == 'renal', "propDist"] = pROC::roc(testRenal$renal, testRenal$propDist)$auc
rocRes[rocRes$phenotype == 'renal', "seedDist"] = pROC::roc(testRenal$renal, testRenal$seedDist)$auc
rocRes[rocRes$phenotype == 'retina', "seedDist"] = pROC::roc(testretina$retina, testretina$seedDist)$auc
rocRes[rocRes$phenotype == 'retina', "propDist"] = pROC::roc(testretina$retina, testretina$propDist)$auc
rocRes[rocRes$phenotype == 'hearing', "propDist"] = pROC::roc(testhearing$hearing, testhearing$propDist)$auc
rocRes[rocRes$phenotype == 'hearing', "seedDist"] = pROC::roc(testhearing$hearing, testhearing$seedDist)$auc
rocRes[rocRes$phenotype == 'polydactyly', "seedDist"] = pROC::roc(testpolydactyly$polydactyly, testpolydactyly$seedDist)$auc
rocRes[rocRes$phenotype == 'polydactyly', "propDist"] = pROC::roc(testpolydactyly$polydactyly, testpolydactyly$propDist)$auc
rocRes[rocRes$phenotype == 'skeletal', "propDist"] = pROC::roc(testskeletal$skeletal, testskeletal$propDist)$auc
rocRes[rocRes$phenotype == 'skeletal', "seedDist"] = pROC::roc(testskeletal$skeletal, testskeletal$seedDist)$auc

rocRes = gather(rocRes, key = 'distType', value = 'dist', -phenotype)

ggplot(rocRes, aes(x=phenotype, y=dist, fill = distType)) + geom_bar(stat = 'identity', position = 'dodge')


for (i in 1:nrow(test)){
  test$comb[i] = sum(test[i,c("renal","CNS", "hearing","polydactyly","skeletal", "retina")]) > 0
} 

pROC::roc(test$comb, test$seedDist)
pROC::roc(test$comb, test$propDist)

##plot phenotype data according to dendrograms
...
clustObject = hclust(distTraits)
phenotype_df = phenotype_df[match(clustObject$labels, rownames(phenotype_df)),]
Heatmap(t(phenotype_df), col = c('white', 'black'), rect_gp = gpar(col = "white", lwd = 2), cluster_columns = clustObject)
